...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (
      ...
      childBlocks.forEach((childBlock, index) => {
        // 只接纳完整 block 对象，避免污染 recordMap 造成 SSR 崩溃
        if (!childBlock || typeof childBlock !== 'object') return

        const normalizedChild =
          childBlock?.value && typeof childBlock.value === 'object'
            ? childBlock
            : { value: childBlock }

        if (
          !normalizedChild?.value ||
          typeof normalizedChild.value !== 'object' ||
          !normalizedChild.value.type
        ) {
          return
        }

        const newBlockId = `${blockId}_child_${index}`
        normalizedChild.value.id =
          normalizedChild.value.id || childBlock.id || newBlockId

        clonedBlock[newBlockId] = normalizedChild
  ...
  // 最终兜底：移除异常 block，并补齐 id，防止 react-notion-x 在 SSR 时 uuidToId(undefined) 崩溃
  Object.keys(clonedBlock || {}).forEach(key => {
    const item = clonedBlock[key]
    ...
    if (typeof item.value.id !== 'string' || item.value.id.length === 0) {
      item.value.id = key
    }
  })
